...
export default {
  data() {
    return {
      ...
      chatList: [
        {
          user: 'helper',
          text: this.$t('chat.notice'),
          isNotice: true,
        },
        {
          user: 'helper',
          text: `${this.$t('chat.notice2')}\n${this.$t('chat.notice3')}\n${this.$t('chat.notice4')}`,
          isNotice: true,
        },
      ],
      inputText: '',
  ...
  methods: {
    ...
    sendLog(text, time) {
      Firebase.database().ref(`/admin/${this.userKey}`).set({
        text,
        user: this.userKey,
        time,
        answer: false,
      ...
      if (window.location.hostname !== 'localhost') {
        ...
        const obj = '{"text": "문의사항이 있습니다. <https://shoong.tours/chat/admin|답변URL> 클릭해서 답변해주세요."}';
        body.set('payload', obj);
        const queryUrl = 'https://hooks.slack.com/services/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';
        Axios.post(queryUrl, body);
      }
    },
    chatSubmit() {
      ...
      Firebase.database().ref(`/chatList/${this.userKey}`).push({
        text,
        user: this.userKey,
        time,
      });
      this.sendLog(text, time);
      this.inputText = '';
    },
    scrollBottom() {
  ...
  mounted() {
    ...
    const init = () => {
      ...
      this.chatSocket = Firebase.database().ref(`/chatList/${this.userKey}`);
      ...
      const answer = Firebase.database().ref(`/admin/${this.userKey}/answer`);
      answer.once('value').then((snap) => {
        if (snap.val() === true) {
          answer.set('read');
        }
      });
